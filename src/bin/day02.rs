...
fn result(opponents_play: i32, my_play: i32, shapes: &[i32; 3]) -> i32 {
    if opponents_play == my_play {
        3
    } else if opponents_play == shapes[((my_play + 1) % 3) as usize] {
        6
    } else {
        0
    }
...
fn get_correct_step(opponents_play: i32, desired_outcome: i32, shapes: &[i32; 3]) -> i32 {
    if desired_outcome == 0 {
        shapes[((opponents_play + 1) % 3) as usize]
    } else if desired_outcome == 3 {
        opponents_play
    } else {
        shapes[(opponents_play % 3) as usize]
    }
}
...
fn main() {
    ...
    let shape_points = HashMap::from([("A", 1), ("X", 1), ("B", 2), ("Y", 2), ("C", 3), ("Z", 3)]);
    ...
    // part two
    let desired_outcome_points = HashMap::from([("X", 0), ("Y", 3), ("Z", 6)]);
